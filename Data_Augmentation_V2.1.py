...
def rotate_image(aglens, angle_rad, imgs_path, anno_new_dir_, img_new_dir):
    ...
    for img_path in imgs_path:
        ...
        for i in range(angle_num):
            ...
            ext = os.path.splitext(img_path)[-1]  # 得到原图的后缀
            # 保存旋转后图像
            new_img_name = 'R%s_%s%s' % (str(aglens[i]).zfill(3), file_name, ext)
            cv2.imwrite(os.path.join(img_new_dir, new_img_name), im_rot)  # 新的命名方式为P+角度+原图名称
            # 读取anno标签数据，返回相应的信息
            anno = os.path.join(anno_path, '%s.xml' % file_name)
            [gts, w, h, d] = readXml(anno)

            anno_new_dir = anno_new_dir_ + '\\' + 'R' +str(aglens[i]).zfill(3) + '_' + file_name   # XML 新名称 修改202111.28
...
if __name__ == '__main__':
    ...
    AUG_mode = 1 # 1 == Rotate代表旋转, 0 == Flip表示翻转['V', 'H']
    
    AUG = 'Rotate' if AUG_mode == 1 else 'Flip'# 数据扩增的方式 1 == Rotate代表旋转, 0 == Flip表示翻转['V', 'H']
    ...
    if AUG == 'Rotate':
        # 旋转角的大小，正数表示逆时针旋转
        angles = [180]  # 角度im_rotate用到的是角度制 [5, 90, 180, 270, 355] # 可用 180  # 其他角度出现一些不可见问题
