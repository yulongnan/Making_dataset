...
aug_rot = get_aug([Rotate(limit=(180,180), always_apply=True, p =1.0)])  # 旋转180
aug_bri = get_aug([RandomBrightness(limit=(-0.5,0.5), always_apply=True, p=1.0)]) # 亮度 
aug_clahe = get_aug([CLAHE(clip_limit=12.0, tile_grid_size=(16, 16), always_apply=True, p=1)]) 
aug_MBlur = get_aug([MotionBlur(blur_limit=7, always_apply=True, p=1)]) 

aug_RGBS = get_aug([RGBShift(r_shift_limit=40, g_shift_limit=40, b_shift_limit=40, always_apply=True, p=1)]) #2021.12.03

aug_GBlur = get_aug([GaussianBlur(blur_limit=75, always_apply=True, p=1)]) #2021.12.03

aug_HueS = get_aug([HueSaturationValue(hue_shift_limit=10, sat_shift_limit=40, val_shift_limit=40, always_apply=True, p=1)])
aug_Cuto = get_aug([Cutout(num_holes=50, max_h_size=80, max_w_size=80, fill_value=1, always_apply=True, p=1)])
aug_Ctra = get_aug([RandomContrast( limit=(-0.6,0.6),always_apply=True,p=1)])

aug_list = [aug_hor, aug_rot, aug_bri, aug_clahe, aug_RGBS, aug_GBlur, aug_Cuto, aug_Ctra ]  # 想用哪个，就添加在找个list里     
aug_list_str = ['HF_','R180_', 'Bri_', 'CLAHE_','RGBS_', 'GBlur_' ,'Cuto_','Ctra_' ]    # 对应于aug_list # 修改2021.11.30



# aug_list = [aug_hor, aug_rot, aug_bri, aug_clahe, aug_RGBS, aug_GBlur ]  # 想用哪个，就添加在找个list里     
# aug_list_str = ['HF_','R180_', 'Bri_', 'CLAHE_','RGBS_', 'GBlur_' ]    # 对应于aug_list # 修改2021.11.30 

Flag_run_or_test = 1   # 1 = run  0 = test  区别在于存放文件是否按 = 类别分开
Flag_copy = 1         # 0=不复制 1=复制 复制原始图片===>图像增强文件夹   
#--------------------------            读取xml，解析，增强图像，修改box信息，写入xml            -----------------------------#
if __name__ == '__main__': 
    path_myadd = '_X1_800'  # 注意后缀名   '_Rb_Sort' 
    ...
    for xml in tqdm(xmls):   #xmls[:3] 小批量测试
        # print('xml_name',xml)
        xmlName = xml.split('.')[0] 
        imgName = xmlName + '.jpg' 
